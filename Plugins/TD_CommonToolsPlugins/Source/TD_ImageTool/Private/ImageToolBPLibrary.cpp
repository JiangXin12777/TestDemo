...
void UImageToolBPLibrary::GetImageFormatFromPath(const FString& Path, EImageFormat& ImageFormat)
{
	...
	if (FileExtension.Equals(TEXT("jpg"), ESearchCase::IgnoreCase)
		|| FileExtension.Equals(TEXT("jpeg"), ESearchCase::IgnoreCase)
		|| FileExtension.Equals(TEXT("jfif"), ESearchCase::IgnoreCase))
	{
...
bool UImageToolBPLibrary::GetWebpImageRawData(const TArray<uint8>& InWebpData, TArray64<uint8>& OutRawImageData, int& OutWidth, int& OutHeight)
{
	if (!WebPGetInfo((const uint8_t*)InWebpData.GetData(), InWebpData.Num(), &OutWidth, &OutHeight)) {
		return false;
	}
	uint8_t* OutBuffer = WebPDecodeBGRA((const uint8_t*)InWebpData.GetData(), InWebpData.Num(), &OutWidth, &OutHeight);
	if (!OutBuffer) {
		WebPFree(OutBuffer);
		return false;
	}
	OutRawImageData = TArray64<uint8>(OutBuffer, OutWidth * OutHeight * 4);
	WebPFree(OutBuffer);
	return true;
}
